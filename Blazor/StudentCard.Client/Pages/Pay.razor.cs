...
{
    [Inject] private JSRuntime _jsRuntime { get; set; } = null!;
    ...
    private PayRequestModel _pay = new();
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        switch (studentsResult)
        {
            ...
            case ErrorResult errorResult :
                _navigationManager.NavigateTo($"/Error/{WebUtility.HtmlEncode(errorResult.Message+"  "+errorResult.Exception)}");
                break;
        }
        
        var agentsResult = await _studentService.GetAllAsync(token);
        switch (agentsResult)
        {
