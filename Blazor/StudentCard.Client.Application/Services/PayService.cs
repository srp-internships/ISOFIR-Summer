...
public class PayService: IPayService
{
    ...
    public async Task<Result> GetAllAsync(string token)
    {
        try
        {
            ...
            var students = await _httpClient.GetFromJsonAsync<List<StudentResponseModel>>("/api/pay/getAll");
    ...
    public async Task<Result> AddAsync(PayRequestModel payDto,string token)
    {
        try
        {
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
            var response = await _httpClient.PostAsJsonAsync("/api/pay/create", payDto);
