...
public class StorageService : IStorageService
{
    ...
    public async Task<Result> CreateOrUpdateAsync(StorageRequestModel storageDto)
    {
        try
        {
            ...
            if (storage == null) return new ErrorResult(new Exception(), "Невозможно обработать ваши данные");
    ...
    public async Task<Result> GetAllAsync(int userId)
    {
        try
        {
            var result = await _storageRepository.GetAllAsync(userId);
            return new OkResult<List<StorageResponseModel>>(result
                .Select(s => _mapper.Map<Storage, StorageResponseModel>(s)).ToList());
        }
        catch (Exception e)
        {
            return new ErrorResult(e);
